...
dfRep_orig <- read.csv(file=here('in', 'kp_relatorios_extracted_annotations_with_icd_preprocessed.csv'),
                  sep=';', stringsAsFactors=F, na.strings = "NA")

names(dfRep_orig) = gsub(pattern = "X.", replacement = "", x = names(dfRep_orig))
...
# arrumando algumas inconsistências - números escritos por extenso na coluna VALOR_REEMBOLSO
temp <- dfRep
temp[temp$Filename == "0473720-05.2015.8.19.0001.txt", "VALOR_REEMBOLSO_PREPROCESSED"] = "6000.0"

temp$VALOR_REEMBOLSO_PREPROCESSED_temp <- temp$VALOR_REEMBOLSO_PREPROCESSED

# somando os valores de reembolso que estão separados
temp <- temp %>% separate(VALOR_REEMBOLSO_PREPROCESSED_temp, c("VALOR_REEMBOLSO_PREPROCESSED_1","VALOR_REEMBOLSO_PREPROCESSED_2"), "\n")

# aqui estou inserindo 0 em valores NA - verificar
temp[is.na(temp)] = 0
temp[temp == ""] = 0
...
# removendo as colunas temporárias que criei para somar
temp$VALOR_REEMBOLSO_PREPROCESSED_1 <- NULL
temp$VALOR_REEMBOLSO_PREPROCESSED_2 <- NULL

# DANO MORAL #

# arrumando algumas inconsistências - números escritos por extenso na coluna VALOR_DANO_MORAL

short_val <- c("20000.0", "20000.0", "30000.0", "60000.0", "60000.0",
               "61000.0", "100000.0", "400000.0", "40000.0",
               "200000.0", "10000.0", "50000.0", "70000.0")

long_val <- c("vinte salários mínimos", "vinte mil reais", "trinta mil reais", "sessenta salários mínimos", "sessenta Salários - Mínimos",
              "sessenta e um salários mínimos", "cem salários mínimos", "quatrocentos salários mínimos", "quarenta salários mínimos",
              "duzentos salários mínimos", "dez mil reais", "cinquenta salários mínimos", "setenta salários mínimos"
              )

names(short_val) <- long_val

temp$VALOR_DANO_MORAL_PREPROCESSED_temp <- temp$VALOR_DANO_MORAL_PREPROCESSED
pos <- which(str_detect(temp$VALOR_DANO_MORAL, "[[:digit:]]|r$") == FALSE)
...
temp <- temp %>% separate(VALOR_DANO_MORAL_PREPROCESSED_temp, c("VALOR_DANO_MORAL_PREPROCESSED_1","VALOR_DANO_MORAL_PREPROCESSED_2"), "\n")
temp[is.na(temp)] = 0
temp[temp == ""] = 0

temp$VALOR_DANO_MORAL_PREPROCESSED_total <- as.numeric(temp$VALOR_DANO_MORAL_PREPROCESSED_1) + as.numeric(temp$VALOR_DANO_MORAL_PREPROCESSED_2)
...
# transformando as colunas em flags
dfRep[dfRep$PEDIU_MEDICAMENTO_EME != "0", "PEDIU_MEDICAMENTO_EME"] <- 1
...
# Se tem valor em VALOR_REEMBOLSO_PREPROCESSED, PEDIU_REEMBOLSO deve ser 1
dfRep[dfRep$VALOR_REEMBOLSO_PREPROCESSED_total != "0", "PEDIU_REEMBOLSO"] <- 1


# separando o CID
dfRep$CID_GRUPO <- gsub("\\s+", ",", gsub("^\\s+|\\s+$", "", dfRep$CID_GRUPO))

dfRep <-  dfRep %>%
  rowwise() %>%
  mutate(CID_GRUPO = paste(unique(str_sort(unlist(strsplit(CID_GRUPO, ",", fixed = TRUE)))), 
                           collapse = ","))
...
# verificando se pediu_reembolso == 1 tem valores
temp <- dfRep %>% filter(PEDIU_REEMBOLSO == "1" & VALOR_REEMBOLSO_PREPROCESSED_total == "0")
...
dfTrial_orig <- read.csv(file=here('in', 'kp_primeira_instancia_extracted_annotations_preprocessed.csv'),
                    sep=';', stringsAsFactors=F, na.strings = "NA")

names(dfTrial_orig) = gsub(pattern = "X.", replacement = "", x = names(dfTrial_orig))
...
# alguns valores VALOR_REEMBOLSO_PREPROCESSED estavam na coluna errada
pos <- which(dfTrial$REEMBOLSO != "0" & grepl("\\d+(?:,\\d+)?", dfTrial$REEMBOLSO))

dfTrial[pos, "VALOR_REEMBOLSO_PREPROCESSED"] <- dfTrial[pos, "REEMBOLSO"]
dfTrial[pos, "REEMBOLSO"] <- "1"

dfTrial[pos, "VALOR_REEMBOLSO_PREPROCESSED"] <- str_extract(dfTrial[pos, "VALOR_REEMBOLSO_PREPROCESSED"], '[0-9]+\\.[0-9]+\\,[0-9]+|[0-9]+\\,[0-9]+')
dfTrial[pos, "VALOR_REEMBOLSO_PREPROCESSED"] <- str_replace(dfTrial[pos, "VALOR_REEMBOLSO_PREPROCESSED"], "\\.", "")
dfTrial[pos, "VALOR_REEMBOLSO_PREPROCESSED"] <- str_replace(dfTrial[pos, "VALOR_REEMBOLSO_PREPROCESSED"], "\\,", ".")
...
temp$VALOR_REEMBOLSO_PREPROCESSED_total <- as.numeric(temp$VALOR_REEMBOLSO_PREPROCESSED_1) + as.numeric(temp$VALOR_REEMBOLSO_PREPROCESSED_2)
...
# arrumando algumas inconsistências - números escritos por extenso na coluna VALOR_DANO_MORAL
short_val <- c("20000.0", "7000.0", "6000.0", "8000.0", "40000.0", "10000.0", "5000.0", "50000.0", "4000.0")

long_val <- c("vinte mil reais", "sete mil reais", "seis mil reais", "oito mil reais", "quarenta mil reais", "dez mil reais", "cinco mil reais", "cinquenta mil reais", "quatro mil reais")

names(short_val) <- long_val

temp$VALOR_DANO_MORAL_PREPROCESSED_temp <- temp$VALOR_DANO_MORAL_PREPROCESSED
pos <- which(str_detect(temp$VALOR_DANO_MORAL, "[[:digit:]]") == FALSE)

temp$VALOR_DANO_MORAL_PREPROCESSED_temp[pos] <- str_replace_all(temp$VALOR_DANO_MORAL[pos] , short_val)
...
# Se VALOR_REEMBOLSO_PREPROCESSED for diferente de 0, REEMBOLSO deve ser 1
dfTrial[dfTrial$VALOR_REEMBOLSO_PREPROCESSED != "0", "REEMBOLSO"] <- 1

saveRDS(dfTrial, file = "in/dfTrial.Rds")

# pegar os valores do reembolso que estavam zerados no pedido
temp <- dfTrial %>% filter(Filename %in% id_Rep_error) %>% select("Filename", "VALOR_REEMBOLSO_PREPROCESSED_total")

dfRep$VALOR_REEMBOLSO_PREPROCESSED_total[dfRep$Filename %in% temp$Filename] <- temp$VALOR_REEMBOLSO_PREPROCESSED_total[temp$Filename %in% dfRep$Filename]
...
dfAppell_orig <- read.csv(file=here('in', 'kp_segunda_instancia_extracted_annotations_preprocessed.csv'),
                     sep=';', stringsAsFactors=F, na.strings = "NA")

names(dfAppell_orig) = gsub(pattern = "X.", replacement = "", x = names(dfAppell_orig))
...
# alguns valores VALOR_REEMBOLSO_PREPROCESSED estavam na coluna errada
pos <- which(dfAppell$REEMBOLSO != "0" & grepl("\\d+(?:,\\d+)?", dfAppell$REEMBOLSO))

dfAppell[pos, "VALOR_REEMBOLSO_PREPROCESSED"] <- dfAppell[pos, "REEMBOLSO"]
dfAppell[pos, "REEMBOLSO"] <- "1"

dfAppell[pos, "VALOR_REEMBOLSO_PREPROCESSED"] <- str_extract(dfAppell[pos, "VALOR_REEMBOLSO_PREPROCESSED"], '[0-9]+\\.[0-9]+\\,[0-9]+|[0-9]+\\,[0-9]+')
dfAppell[pos, "VALOR_REEMBOLSO_PREPROCESSED"] <- str_replace(dfAppell[pos, "VALOR_REEMBOLSO_PREPROCESSED"], "\\.", "")
dfAppell[pos, "VALOR_REEMBOLSO_PREPROCESSED"] <- str_replace(dfAppell[pos, "VALOR_REEMBOLSO_PREPROCESSED"], "\\,", ".")
...
# algumas entradas não aparecem no relatório mas aparecem na 1a
...
# se reembolso == 1 em Trial, então PEDIU_REEMBOLSO == 1 em Report
...
# se EXCLUIU == 0 e VALOR == 0, pegar valor da 1a
temp <- dfAppell %>% filter(EXCLUIU == 0 & VALOR_DANO_MORAL_PREPROCESSED_total == "0")
...
# se EXCLUIU == 1 o VALOR_DANO_MORAL_PREPROCESSED_total deve ser 0,
temp <- dfAppell %>% filter(EXCLUIU == 1 & VALOR_DANO_MORAL_PREPROCESSED_total != "0")
id_Appell_error <- temp$Filename

dfAppell[dfAppell$Filename %in% id_Appell_error, "VALOR_DANO_MORAL_PREPROCESSED_total"] <- "0"
...
temp_1 <- dfRep %>% select(Filename, VALOR_DANO_MORAL_PREPROCESSED_total)
temp_1$document <- "R"
colnames(temp_1) <- c("Filename", "value", "document")
temp_2 <- dfTrial %>% select(Filename, VALOR_DANO_MORAL_PREPROCESSED_total)
temp_2$document <- "1a"
colnames(temp_2) <- c("Filename", "value", "document")
temp_3 <- dfAppell %>% select(Filename, VALOR_DANO_MORAL_PREPROCESSED_total)
temp_3$document <- "2a"
colnames(temp_3) <- c("Filename", "value", "document")

dfRepTrialAppell_CIGgrouped_dm <- rbind(temp_1, temp_2, temp_3)

omitted_val <- dfRepTrialAppell_CIGgrouped_dm %>% filter(document == "R" & value == "0")
...
dfAppellExcluiu <- dfAppellbyCID %>% filter(EXCLUIU == 1) %>% select(Filename)
...
dfCIDS <- read.csv(file=here('in', "CID-10-CAPITULOS.CSV"), sep=";")
...
# substitui os valores do CID pelo nome
...
#só casos que tiveram primeira instância por CID
dfJustFirst <- dfTrialbyCID %>% 
  filter(Filename %in% dfRepbyCID$Filename & !(Filename %in% dfAppellbyCID$Filename))
temp <- dfJustFirst %>% group_by(CID_GRUPO) %>% tally()


#casos que tiveram segunda instância
dfWithSecond <- dfTrialbyCID %>% 
  filter(Filename %in% dfRepbyCID$Filename & Filename %in% dfAppellbyCID$Filename)
temp <- dfWithSecond %>% group_by(CID_GRUPO) %>% tally()


dfRepbyCID_dm <- dfRepbyCID %>% select("Filename", "VALOR_DANO_MORAL_PREPROCESSED_total", "CID_GRUPO", "CID_NOME")
dfRepbyCID_dm$document <- "R"
colnames(dfRepbyCID_dm) <- c("Filename", "VALOR_DANO_MORAL_PREPROCESSED_total", "CID_GRUPO", "CID_NOME", "document")

dfTrialbyCID_dm <- dfTrialbyCID %>% select("Filename", "VALOR_DANO_MORAL_PREPROCESSED_total", "CID_NOME", "CID_GRUPO")
dfTrialbyCID_dm$document <- "1a"
documents_trial_no_dn <- dfTrialbyCID %>% filter(VALOR_DANO_MORAL_PREPROCESSED_total == "0")

dfAppellbyCID_dm <- dfAppellbyCID %>% select("Filename", "VALOR_DANO_MORAL_PREPROCESSED_total", "CID_NOME", "CID_GRUPO")
dfAppellbyCID_dm$document <- "2a"

saveRDS(dfAppellbyCID_dm, file = "in/dfAppellbyCID_dm.Rds")


dfRepTrialAppell_dm <- rbind(dfRepbyCID_dm, dfTrialbyCID_dm, dfAppellbyCID_dm)

dfRepTrialAppell_dm <- dfRepTrialAppell_dm %>% gather(key = "type", value = "value", -c(Filename, CID_GRUPO, CID_NOME, document))

dfRepTrialAppell_dm$value <- as.numeric(dfRepTrialAppell_dm$value)

# omitted_val <- dfRepTrialAppell_dm %>% filter(document == "R" & value == "0")
